...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($FirstName) && !empty($LastName) && !empty($EmailID) && !empty($PhoneNumber) && !empty($Country) && !empty($PostCode) && !empty($Address) && !empty($LinkedInID) && !empty($photoNewName)) {
        $host = "localhost";
        $dbUsername = "root";
        $dbPassword = "";
        $dbName = "test";
        ...
        if (mysqli_connect_error()) {
            ...
            $INSERT = "INSERT INTO first (FirstName, LastName, EmailID, PhoneNumber, Country, PostCode, Address, LinkedInID, Photo) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($INSERT);
            $stmt->bind_param("sssssssss", $FirstName, $LastName, $EmailID, $PhoneNumber, $Country, $PostCode, $Address, $LinkedInID, $photoNewName);
            $stmt->execute();
...
<body id="body">
    <form id="mainform" action="first.php" method="post"  enctype="multipart/form-data">
        <div id="new1">
    ...
    <script>
        function myfunction(){
            ...
            if(!photo){
            photoError.innerHTML = "Upload a Photo";
            isValid = false;
            } else {
